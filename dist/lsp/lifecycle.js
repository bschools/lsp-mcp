...
export async function createLspLifecycle(workspaceRoot) {
    ...
    async function didOpen(filePath) {
    ...
    async function didChange(filePath) {
        const fileUri = url.pathToFileURL(filePath).href;
        const stat = fs.statSync(filePath);
        const text = fs.readFileSync(filePath, "utf8");
        const info = openFiles.get(fileUri);
    ...
    async function didClose(filePath) {
        const fileUri = url.pathToFileURL(filePath).href;
        if (!openFiles.has(fileUri))
            return;
