...
describe("find_references integration", () => {
  ...
  beforeEach(async () => {
    workspace = fs.mkdtempSync(path.join(os.tmpdir(), "find-refs-"));
  ...
  it("finds declaration + references across fixture files", async () => {
    ...
    const result = JSON.parse(content) as {
      files: Array<{ path: string; ranges: unknown[] }>;
    };

    const paths = result.files.map((f) => path.basename(f.path));
    expect(paths).toContain("user.service.ts");
    expect(paths).toContain("consumer.ts");
    expect(paths).toContain("user.service.spec.ts");
  }, 60000);
});
