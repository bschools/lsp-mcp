...
describe("hover integration", () => {
  ...
  beforeEach(async () => {
    workspace = fs.mkdtempSync(path.join(os.tmpdir(), "hover-"));
  ...
  it("returns hover info for a class name", async () => {
    // UserService is declared at line 0, col 13 in user.service.ts
    const target = path.join(workspace, "src", "user.service.ts");

    const resp = await client.request("tools/call", {
      name: "hover",
      arguments: { filePath: target, line: 0, column: 13 },
    });

    expect(resp.error).toBeUndefined();
    const content = (resp.result as { content: { text: string }[] }).content[0].text;
    const result = JSON.parse(content) as { hover: { contents: unknown } | null };

    expect(result.hover).not.toBeNull();
  }, 60000);

  it("returns null hover for whitespace position", async () => {
    const target = path.join(workspace, "src", "user.service.ts");

    const resp = await client.request("tools/call", {
      name: "hover",
      arguments: { filePath: target, line: 1, column: 0 },
    });

    expect(resp.error).toBeUndefined();
    const content = (resp.result as { content: { text: string }[] }).content[0].text;
    const result = JSON.parse(content) as { hover: unknown };
    // hover may be null or have contents — just check no protocol error
